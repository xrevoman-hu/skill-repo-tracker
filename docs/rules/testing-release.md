...
生成清单前再次检查工作树干净，然后输出 SHA-256、旁置 `.release.json` 与一个 unsigned
base64url manifest token。token 只把操作者交接的 version/commit/name/bytes/SHA 字段放在
同一载体中，避免逐字段混用；它没有签名，不能证明是谁生成，也不能单独证明 local gate
确实执行过。远端 phase 必须显式接收这个 token，再独立核对 main/tag/Release/digest 并
