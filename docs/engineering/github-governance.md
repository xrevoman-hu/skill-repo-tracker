...
`scripts/`、workflow、Rules、ADR、测试/编译配置、Tauri 权限或其他治理事实源，Check Run 只有在
当前 head 上发生明确的 `governance-reviewed` labeled event 时成功。测试 selector 即使不变，只要
evidence 文件内容或 rename 前文件名发生变化也必须复审；guard 始终不读取 PR 文件内容。`synchronize`、
`edited`、`reopened` 与 `unlabeled` 会重新评估并尝试发布 failure；无关 label event 不能代替
复审。guard 同时复核 API 返回的 `head.sha`、base ref 与事件值，因此新 success 只能发布到
本次 API 复核的精确 head。CI 另行订阅 `pull_request.edited`，title/body 修改后会重跑 PR
evidence。

`pull_request_target` 自身的 `GITHUB_SHA` 是默认分支提交，不能直接满足“最新 PR commit 上的
required check”。因此 trusted base 脚本通过 Checks API 在事件携带、并由 Pull Request API
复核过的 `head.sha` 上创建固定名 `Trusted policy / guard` 的 completed Check Run。workflow
权限严格为 `contents: read`、`pull-requests: read`、`checks: write`；唯一写能力是创建这个
Check Run，不允许写 contents、issues、pull requests 或 Actions。API、输入或策略判断异常时
会尝试发布 failure。

这里有一条不能靠仓库代码消除的边界：GitHub Check Run 按 commit 记录。如果同一 head 已有
success，而后续撤销事件的 failure 因 API/权限故障没有成功创建，旧 success 不会被原子撤销。
此外，Ruleset 即使把 context 绑定到 GitHub Actions integration，也不能隔离同仓库另一份
拥有 `checks: write` 的 workflow。于是这套方案是默认分支代码驱动的流程加固，不是密码学
意义上的“不可绕过信任根”。需要强信任根时，应使用凭据隔离的专用 GitHub App，或组织级、
从独立受控仓库提供的 required workflow，并将 ruleset 的 `integration_id` 绑定到该独立主体。
...
[Checks API 的 `checks: write` 要求](https://docs.github.com/en/rest/checks/runs?apiVersion=2022-11-28#create-a-check-run)、
...
3. 启用 Require status checks，并选择 `CI / verify`、`CI / coverage`、`CI / msrv` 和
   ...
   `context` 字段。最后一个是 base script 在 PR head 上创建的精确 Check Run 名称。
...
3. 创建一个普通、不合并的 probe PR，确认 Check Run `Trusted policy / guard` 出现在该 PR
   最新 head SHA；再触及一个 critical path，确认无 label 失败、添加
   `governance-reviewed` 后成功；记录该 Check Run 的实际 App/integration。
...
target=Branch 的 active ruleset，且该 ruleset 保护 `main`、`bypass_actors` 明确为空、PR 必经且 approvals 精确为 0、四个且仅
