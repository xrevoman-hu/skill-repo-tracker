...
const APP_USER_AGENT: &str = concat!("SkillRepoTracker/", env!("CARGO_PKG_VERSION"));
...
struct AppState {
    ...
    credential_lock: github_accounts::CredentialMutationLock,
...
impl AppState {
    fn new(data_dir: PathBuf) -> Result<Self, AppError> {
        Self::new_with_adapters(data_dir, AppAdapters::default())
    }

    fn new_with_adapters(data_dir: PathBuf, adapters: AppAdapters) -> Result<Self, AppError> {
        ...
        let credential_lock = github_accounts::CredentialMutationLock::new(&data_dir)?;
...
pub enum SkillActionOutcome {
    ...
    Conflict {
        skills: Vec<UiSkill>,
        conflict: Box<SkillUpdateConflict>,
    },
}
...
#[derive(Debug, Deserialize)]
#[serde(rename_all = "camelCase")]
pub struct GithubAccountTokenRequest {
    token: String,
}

#[derive(Debug, Deserialize)]
#[serde(rename_all = "camelCase")]
pub struct GithubAccountRequest {
    account_id: String,
}

#[derive(Debug, Deserialize)]
#[serde(rename_all = "camelCase")]
pub struct GithubCatalogRequest {
    account_id: Option<String>,
}

#[derive(Debug, Deserialize)]
#[serde(rename_all = "camelCase")]
pub struct GithubStarRequest {
    account_id: String,
    owner: String,
    repo: String,
    starred: bool,
...
fn migrate_legacy_schema(conn: &Connection) -> Result<(), AppError> {
    conn.execute_batch(
        r#"
        ...
        CREATE TABLE IF NOT EXISTS github_accounts (
          ...
          status TEXT NOT NULL DEFAULT 'saved_unverified',
        ...
        CREATE TABLE IF NOT EXISTS github_repo_catalog (
    ...
    add_column_if_missing(
        ...
        "repositories",
        "github_account_id",
        "ALTER TABLE repositories ADD COLUMN github_account_id TEXT",
    )?;
    add_column_if_missing(
        conn,
        "repositories",
...
fn github_account_id_for_login(login: &str) -> String {
    format!("github:{}", slugify(&login.to_ascii_lowercase()))
}

fn github_account_from_row(row: &rusqlite::Row<'_>) -> rusqlite::Result<GithubAccountRecord> {
...
fn github_account_by_id(
    conn: &Connection,
    account_id: &str,
) -> Result<Option<GithubAccountRecord>, AppError> {
    conn.query_row(
...
fn github_auth_configured(conn: &Connection) -> bool {
...
fn load_ui_github_accounts(conn: &Connection) -> Result<Vec<UiGithubAccount>, AppError> {
    ...
    for row in rows {
        accounts.push(ui_github_account(row?));
    }
    Ok(accounts)
}
...
fn record_temp_cleanup_task(conn: &Connection, report: &CleanupReport) -> Result<bool, AppError> {
    ...
    if report.failed > 0 {
        insert_retryable_task(
            ...
            TaskWrite {
                id: &task_id,
                kind: "Temp artifact cleanup",
                target: "Configured Skill roots",
                progress: &progress,
                status,
...
fn validate_directory_path(kind: &str, raw_path: &str) -> DirectoryValidation {
    ...
    validate_directory_path_after_creation(kind, raw_path, expanded, exists, create_result)
}

fn validate_directory_path_after_creation(
    kind: &str,
    raw_path: &str,
    expanded: PathBuf,
    exists: bool,
    ...
    let writable = create_result
        ...
        .map(|_| {
            tempfile::Builder::new()
                .prefix(".skill-repo-tracker-write-test-")
                .tempfile_in(&expanded)
                .and_then(|mut probe| {
...
fn settings_error_with_directory_cleanup(
    mut error: AppError,
    created_directories: &[CreatedSettingsDirectory],
) -> AppError {
    let cleanup_errors = cleanup_created_settings_directories(created_directories);
    if cleanup_errors.is_empty() {
        return error;
    }
    let cleanup_details = format!(
        "settings directory rollback could not remove every newly created directory: {}",
        cleanup_errors.join("; ")
    );
    error.details = Some(match error.details {
        Some(details) => format!("{details}; {cleanup_details}"),
...
fn insert_task(conn: &Connection, task: TaskWrite<'_>) -> Result<(), AppError> {
    let now = utc_now();
    conn.execute(
    ...
    for (index, line) in task.log.iter().enumerate() {
        conn.execute(
            ...
            params![task.id, index as i64 + 1, line],
        )?;
    }
    Ok(())
}
...
fn insert_retryable_task<T: Serialize>(
    ...
    conn.execute(
        "UPDATE backup_jobs
         ...
         WHERE id = ?1",
        params![task_id, retry_action, payload],
    )?;
    Ok(())
}
...
fn insert_failed_task(
    ...
    let _ = insert_task(
        conn,
        TaskWrite {
            id: &temp_artifacts::unique_operation_id(id_prefix),
            kind,
            target,
            progress: "0 / 1",
            status: "failed",
...
fn scopes_from_headers(headers: &HeaderMap) -> String {
    headers
        .get("x-oauth-scopes")
...
fn repository_permission_summary(value: &serde_json::Value) -> String {
        ...
        .unwrap_or_else(|| {
            (
                repo.get("owner")
                    ...
                    .unwrap_or("")
                    .to_string(),
                json_string(repo, "name"),
            )
        });
    if owner.is_empty() || name.is_empty() {
        return Ok(());
    }
    let visibility = json_string(repo, "visibility");
    conn.execute(
        ...
        params![
            ...
            owner,
            name,
            repo.get("id").and_then(|value| value.as_i64()).unwrap_or(0),
            json_string(repo, "html_url"),
            json_string(repo, "description"),
            if visibility.is_empty() {
                if repo.get("private").and_then(|value| value.as_bool()).unwrap_or(false) {
                    "private".to_string()
                } else {
                    "public".to_string()
                }
            ...
            if repo.get("private").and_then(|value| value.as_bool()).unwrap_or(false) { 1 } else { 0 },
            if repo.get("fork").and_then(|value| value.as_bool()).unwrap_or(false) { 1 } else { 0 },
            if repo.get("archived").and_then(|value| value.as_bool()).unwrap_or(false) { 1 } else { 0 },
            json_string(repo, "default_branch"),
...
fn refresh_tracked_catalog_links(conn: &Connection) -> Result<(), AppError> {
    conn.execute(
        "UPDATE github_repo_catalog
         SET last_refreshed = last_refreshed
         WHERE full_name IN (SELECT owner || '/' || repo FROM repositories WHERE source_type = 'github')",
        [],
    )?;
    Ok(())
}

fn load_ui_github_repositories(
    conn: &Connection,
    account_id: Option<&str>,
) -> Result<Vec<UiGithubRepository>, AppError> {
        ...
        "SELECT c.account_id, a.login, c.owner, c.repo, c.full_name, c.html_url,
                ...
                (SELECT r.id FROM repositories r
                 WHERE r.source_type = 'github'
                   ...
                   AND lower(r.repo) = lower(c.repo)
                 ORDER BY r.updated_at DESC
                 LIMIT 1) AS tracked_repo_id
         FROM github_repo_catalog c
         JOIN github_accounts a ON a.id = c.account_id",
    );
    if account_id.is_some() {
        sql.push_str(" WHERE c.account_id = ?1");
    }
    sql.push_str(" ORDER BY c.starred DESC, c.github_updated_at DESC, c.full_name ASC");
    let mut stmt = conn.prepare(&sql)?;
    let map_row = |row: &rusqlite::Row<'_>| {
        Ok(UiGithubRepository {
...
async fn fetch_github_readme_search(
    transport: &dyn adapters::GithubHttpAdapter,
    owner: &str,
    repo: &str,
    ref_name: &str,
    token: Option<&str>,
...
fn sync_local_skills(conn: &Connection, request: LocalSkillSync<'_>) -> Result<(), AppError> {
    let now = utc_now();
    let mut seen = Vec::new();
    for scan in request.scans {
        let id = skill_id(request.repo_id, &scan.path);
        seen.push(id.clone());
        let skill_dir = if scan.path == "." {
            request.root.to_path_buf()
        } else {
            request.root.join(&scan.path)
        };
        let installed_hash = if request.installed {
            Some(hash_directory(&skill_dir)?)
        } else {
            None
        };
        let status = if request.installed {
            "installed-latest"
        } else {
            "not-installed"
        };
        conn.execute(
            ...
            params![
                ...
                scan.path,
                request.ref_name,
                if request.installed { Some(scan.version.as_str()) } else { None },
                scan.version,
                status,
                if request.installed { 1 } else { 0 },
                now,
                installed_hash,
                request.source_type,
                path_string(&skill_dir),
                if request.installed { Some(path_string(&skill_dir)) } else { None },
                scan.search_text,
            ],
        )?;
    }
    ...
    let rows = stmt.query_map(params![request.repo_id], |row| row.get::<_, String>(0))?;
    for row in rows {
        let existing_id = row?;
        if !seen.contains(&existing_id) {
            conn.execute(
...
fn save_local_repository_with_plugins(
    ...
    sync_local_skills(
        ...
        LocalSkillSync {
            ...
            root: &canonical_root,
            scans,
            source_type: if installed_library {
                "installed_local"
            } else {
                "local_repo"
            },
...
#[expect(clippy::too_many_arguments, reason = "explicit task audit boundary")]
...
fn set_waiting_conflict_task_sync_result(
    conn: &Connection,
    result: ConflictTaskResult<'_>,
) -> Result<(), AppError> {
    let retry_payload = if result.sync_failed {
        Some(
            serde_json::to_string(&SkillActionRequest {
                skill_id: result.skill_id.to_string(),
            })
            .map_err(|error| {
                AppError::with_details(
                    "retry_payload_invalid",
    ...
    conn.execute(
        ...
        params![
            ...
            result.status,
            result.summary,
            if result.sync_failed { 1 } else { 0 },
            result.sync_failed.then_some(RETRY_SYNC_SKILL),
            retry_payload,
            result.completed_at
        ],
    )?;
    ...
    let base = {
        ...
        let result = db.query_row(
            "SELECT s.id, s.name, s.description, s.repo_name, s.path, s.ref_name,
                    ...
                    r.owner, r.repo, r.ref_name, r.github_account_id
             FROM skills s
...
fn get_settings(state: State<'_, AppState>) -> ApiResponse<AppSettings> {
    let db = state.db.lock().expect("db mutex poisoned");
    match settings_from_db(&db, github_auth_configured(&db)) {
...
fn update_settings(
    ...
    let result = update_settings_with_resources(request, &state.db, &state.filesystem_lock);
    match result {
        Ok(settings) => ApiResponse::ok(settings),
        Err(error) => api_err(error),
    }
...
fn update_settings_with_resources(
    request: UpdateSettingsRequest,
    database: &Mutex<Connection>,
    filesystem_lock: &FilesystemMutationLock,
) -> Result<AppSettings, AppError> {
    let changes_filesystem_roots = request.backup_root.is_some()
        || request.skill_library_root.is_some()
        || request.skills_root.is_some();
    let _filesystem_guard = if changes_filesystem_roots {
        Some(filesystem_lock.try_acquire()?.ok_or_else(|| {
            AppError::new(
                "filesystem_busy",
                "另一项文件操作正在进行，目录设置未更改。",
            )
    ...
    let requested_library_root = request
        .skill_library_root
        .as_deref()
        .or(request.skills_root.as_deref());
    for (kind, raw_path, code, message) in [
        (
            "backupRoot",
            request.backup_root.as_deref(),
            "backup_root_unwritable",
            "备份目录不可写，请选择其他目录。",
        ),
        (
            "skillLibraryRoot",
            requested_library_root,
        ...
        let expanded = expand_tilde(raw_path);
        let exists = expanded.exists();
        let create_result = create_settings_directory_tree(&expanded, &mut created_directories);
        let validation =
            validate_directory_path_after_creation(kind, raw_path, expanded, exists, create_result);
        if !validation.writable {
            return Err(settings_error_with_directory_cleanup(
                AppError::with_details(code, message, validation.message),
                &created_directories,
            ));
        }
    }
    ...
    let result = (|| -> Result<AppSettings, AppError> {
        if let Some(value) = request.backup_root {
            set_setting(&transaction, "backup_root", value)?;
        }
        let requested_library_root = request.skill_library_root.or(request.skills_root);
        if let Some(value) = requested_library_root {
            let mut roots_to_remember = get_setting(&transaction, "skill_library_root")?
                .or(get_setting(&transaction, "skills_root")?)
                .into_iter()
                .map(|root| expand_tilde(&root))
                .collect::<Vec<_>>();
            roots_to_remember.push(expand_tilde(&value));
            remember_skill_library_roots(&transaction, &roots_to_remember)?;
            set_setting(&transaction, "skill_library_root", value.clone())?;
            set_setting(&transaction, "skills_root", value)?;
        }
        if let Some(value) = request.default_sync_targets {
            set_setting(
                &transaction,
                "default_sync_targets",
                serialize_sync_targets(&value),
            )?;
        }
        if let Some(value) = request.sync_backup_keep {
            set_setting(&transaction, "sync_backup_keep", value.clamp(1, 50))?;
        }
        if let Some(value) = request.auto_check_interval {
            set_setting(&transaction, "auto_check_interval", value.clamp(15, 1440))?;
        }
        if let Some(value) = request.auto_check_enabled {
            set_setting(&transaction, "auto_check_enabled", value)?;
        }
        if let Some(value) = request.auto_backup_enabled {
            set_setting(&transaction, "auto_backup_enabled", value)?;
        }
        settings_from_db(&transaction, github_auth_configured(&transaction))
    })();
    match result {
        Ok(settings) => match transaction.commit() {
            Ok(()) => Ok(settings),
            Err(error) => Err(settings_error_with_directory_cleanup(
                error.into(),
                &created_directories,
            )),
        },
        Err(mut error) => {
            if let Err(rollback_error) = transaction.rollback() {
                let rollback_details = format!("settings rollback failed: {rollback_error}");
                error.details = Some(match error.details {
                    Some(details) => format!("{details}; {rollback_details}"),
                    None => rollback_details,
                });
            }
            Err(settings_error_with_directory_cleanup(
                error,
                &created_directories,
            ))
        }
...
async fn pick_directory(
    app: AppHandle,
    #[expect(non_snake_case, reason = "stable Tauri wire argument")] defaultPath: Option<String>,
...
async fn add_repository(
    ...
    let result = (|| -> Result<Vec<UiRepository>, AppError> {
        ...
        insert_retryable_task(
            &db,
            TaskWrite {
                id: &temp_artifacts::unique_operation_id("scan"),
                kind: "Scan repository",
                target: &remote.full_name,
                progress: "1 / 1",
                status: "success",
                summary: if scans.is_empty() {
...
async fn check_repositories(
    ...
    let result = (|| -> Result<Vec<UiRepository>, AppError> {
        insert_retryable_task(
            &db,
            TaskWrite {
                id: &temp_artifacts::unique_operation_id("check"),
                ...
                summary: &format!("{success} success, {failed} failed, {skipped} skipped"),
                backup_dir: None,
                log: &log,
            },
...
fn persist_skill_update_conflict(
    ...
    insert_task(
        ...
        TaskWrite {
            ...
            progress: "0 / 1",
            status: "waiting-user",
            summary: "检测到本地修改，等待用户使用 Agent 工具处理",
            backup_dir: None,
            log: &[
...
fn finalize_skill_update_conflict(
    ...
    set_waiting_conflict_task_sync_result(
        ...
        ConflictTaskResult {
            ...
            completed_at: &now,
            sync_failed: sync_report.failure_count > 0,
        },
    )?;
...
async fn verify_skill_update_conflict(
    ...
    let repo = {
        ...
        match load_repository(&db, &skill.repo_id) {
            Ok(Some(repo)) => repo,
            Ok(None) => {
                return Ok(api_err(AppError::new(
                    "github_not_found",
...
fn scan_local_skills(
    ...
    let result = (|| -> Result<Vec<UiSkill>, AppError> {
        ...
        insert_retryable_task(
            &db,
            TaskWrite {
                id: &temp_artifacts::unique_operation_id("local-scan"),
                kind: "Scan local Skills",
                target: &path_string(&root),
                progress: &format!("{} / {}", scans.len(), scans.len()),
                status: "success",
                summary: &format!("{} local Skills scanned", scans.len()),
                backup_dir: None,
                log: &[
...
fn add_local_repository(
    ...
    let result = (|| -> Result<Vec<UiRepository>, AppError> {
        ...
        insert_retryable_task(
            &db,
            TaskWrite {
                id: &temp_artifacts::unique_operation_id("local-repo"),
...
fn delete_skill(
    ...
    let result = (|| -> Result<Vec<UiSkill>, AppError> {
        ...
        insert_task(
            &db,
            TaskWrite {
                id: &temp_artifacts::unique_operation_id("delete-skill"),
                kind: "Delete Skill",
                target: &skill.name,
                progress: &format!(
                    "{} / {}",
                ...
                status: sync_task_status(&sync_report),
                summary: if sync_report.failure_count > 0 {
...
fn restore_skill(
    ...
    let result = (|| -> Result<Vec<UiSkill>, AppError> {
        ...
        insert_task(
            &db,
            TaskWrite {
                id: &temp_artifacts::unique_operation_id("restore-skill"),
                kind: "Restore Skill",
                target: &name,
                progress: &format!(
                    "{} / {}",
...
fn sync_installed_skills(state: State<'_, AppState>) -> ApiResponse<Vec<UiSkill>> {
    ...
    let result = (|| -> Result<Vec<UiSkill>, AppError> {
        ...
        insert_retryable_task(
            &db,
            TaskWrite {
                id: &temp_artifacts::unique_operation_id("sync-skills"),
                ...
                status: sync_task_status(&aggregate),
                summary: &sync_task_summary(&aggregate),
                backup_dir: None,
                log: &aggregate.log,
            },
...
fn update_skill_sync_targets(
    ...
    let result = (|| -> Result<Vec<UiSkill>, AppError> {
        ...
        insert_retryable_task(
            &db,
            TaskWrite {
                id: &temp_artifacts::unique_operation_id("sync-targets"),
                kind: "Update Skill sync targets",
                target: &skill.name,
                progress: &format!(
                    "{} / {}",
                    report.success_count,
                ...
                status: sync_task_status(&report),
                summary: &sync_task_summary(&report),
                backup_dir: None,
                log: &report.log,
            },
...
#[tauri::command]
fn list_github_accounts(state: State<'_, AppState>) -> ApiResponse<Vec<UiGithubAccount>> {
    let db = state.db.lock().expect("db mutex poisoned");
    match load_ui_github_accounts(&db) {
...
#[tauri::command]
async fn save_github_account_token(
    request: GithubAccountTokenRequest,
    state: State<'_, AppState>,
    ...
    let (account, _) = match validate_token_identity(state.adapters.github.as_ref(), &token).await {
        Ok(account) => account,
        Err(error) => return Ok(api_err(error)),
    };
    let db = state.db.lock().expect("db mutex poisoned");
    let result = github_accounts::save_validated_account(
        &db,
...
#[tauri::command]
async fn validate_github_account(
    ...
    let (verified, _) = match validate_token_identity(state.adapters.github.as_ref(), token).await {
        Ok(account) => account,
        Err(error) => return Ok(api_err(error)),
    };
    let db = state.db.lock().expect("db mutex poisoned");
    let result = github_accounts::refresh_validated_account(
        &db,
        &state.credential_lock,
        &request.account_id,
        verified,
...
fn delete_github_account(
    ...
    let db = state.db.lock().expect("db mutex poisoned");
    let result = github_accounts::delete_account(
        &db,
        &state.credential_lock,
        state.adapters.credentials.as_ref(),
        TOKEN_SERVICE,
...
#[tauri::command]
fn list_github_repository_catalog(
    request: GithubCatalogRequest,
    state: State<'_, AppState>,
) -> ApiResponse<Vec<UiGithubRepository>> {
    let db = state.db.lock().expect("db mutex poisoned");
    match load_ui_github_repositories(&db, request.account_id.as_deref()) {
        Ok(repos) => ApiResponse::ok(repos),
        Err(error) => api_err(error),
    }
}

#[tauri::command]
async fn refresh_github_repositories(
    request: GithubCatalogRequest,
    state: State<'_, AppState>,
) -> CommandResult<Vec<UiGithubRepository>> {
    let accounts = {
        let db = state.db.lock().expect("db mutex poisoned");
        let selected: Result<Vec<GithubAccountRecord>, AppError> =
    ...
    for account in &accounts {
        ...
        let repos = match github_response::fetch_json_array_paginated(
            state.adapters.github.as_ref(),
            repo_url,
            token,
            auth.label(),
        ...
        let starred = match github_response::fetch_json_array_paginated_with_accept(
            state.adapters.github.as_ref(),
            starred_url,
            token,
        ...
        let result = (|| -> Result<(), AppError> {
            ...
            db.execute(
                "UPDATE github_accounts SET status = 'verified', last_verified = ?2, updated_at = ?2 WHERE id = ?1",
                params![account.id, utc_now()],
            )?;
            Ok(())
        })();
    ...
    let db = state.db.lock().expect("db mutex poisoned");
    match load_ui_github_repositories(&db, request.account_id.as_deref()) {
        Ok(repos) => Ok(ApiResponse::ok(repos)),
        Err(error) => Ok(api_err(error)),
    }
}

#[tauri::command]
async fn set_github_star(
    ...
    if let Err(error) = set_star_remote(
        state.adapters.github.as_ref(),
        &request.owner,
        &request.repo,
        request.starred,
    ...
    let result = (|| -> Result<Vec<UiGithubRepository>, AppError> {
        ...
        load_ui_github_repositories(&db, Some(&request.account_id))
    })();
    Ok(match result {
        Ok(repos) => ApiResponse::ok(repos),
        Err(error) => api_err(error),
...
#[tauri::command]
async fn add_repository_from_github(
    ...
    let result = (|| -> Result<Vec<UiRepository>, AppError> {
        ...
        db.execute(
            "UPDATE github_repo_catalog
             SET last_refreshed = ?4
             WHERE account_id = ?1 AND lower(owner) = lower(?2) AND lower(repo) = lower(?3)",
            params![request.account_id, remote.owner, remote.repo, utc_now()],
        )?;
        let _ = id;
        load_ui_repositories(&db)
    })();
...
async fn list_prompts(
    ...
    let control = state
        .prompt_search
        .begin(Arc::new(conn.get_interrupt_handle()));
    let (finished_tx, watchdog) =
        prompt_search::spawn_watchdog(control.clone(), Duration::from_secs(1));
    let outcome =
        tauri::async_runtime::spawn_blocking(move || prompts::list_prompts(&conn, &request)).await;
    let _ = finished_tx.send(());
    let _ = watchdog.join();
    state.prompt_search.finish(&control);
    let result = match outcome {
        Ok(Ok(page)) => Ok(page),
        Ok(Err(error)) => Err(control.classify_failure(error)),
...
fn open_backup_folder(
    request: backup_paths::OpenBackupFolderRequest,
    state: State<'_, AppState>,
) -> ApiResponse<String> {
    let db = state.db.lock().expect("db mutex poisoned");
    let target = match backup_paths::resolve_backup_folder(
        &db,
        request.repository_id.as_deref(),
        state.adapters.filesystem.as_ref(),
    ) {
        Ok(target) => target,
