...
- 修复仓库表头全选仅选中当前页的问题，现在覆盖当前搜索、筛选结果的所有页。
- 翻页和排序保留选择，表头半选状态反映完整筛选范围；取消全选保留范围外选择。
- 备份仍需确认，仅 GitHub 来源仓库可选。详见[版本说明](docs/releases/v1.2.9.md)。
...
没有 Apple Developer ID 时，本项目只支持明确披露的 ad-hoc 测试分发。发布者的唯一可执行
合同、local/remote phase、manifest 交接与停止条件只维护在
[`docs/rules/testing-release.md`](docs/rules/testing-release.md)；README 不复制操作命令，避免
发布流程分叉。
...
The repository header checkbox now selects all pages of the current search and filters. Selection survives paging and sorting, and the header mixed state reflects the full result. Unchecking preserves selections outside the filter. Backups still require confirmation and only GitHub repositories are selectable. See the [release notes](docs/releases/v1.2.9.md).
...
Free test distribution without an Apple Developer ID is explicitly ad-hoc. The sole executable
operator contract for its local/remote phases, manifest handoff, and stop conditions lives in
[`docs/rules/testing-release.md`](docs/rules/testing-release.md); this README intentionally does
not duplicate release commands.
